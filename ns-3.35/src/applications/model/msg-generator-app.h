...
/* CDF distribution */
struct cdf_table_msg
{
    struct cdf_entry_msg *entries;
...
void init_cdf_msg(struct cdf_table_msg *table);

/* free resources of a CDF distribution */
void free_cdf_msg(struct cdf_table_msg *table);

/* get CDF distribution from a given file */
void load_cdf_msg(struct cdf_table_msg *table, const char *file_name);

/* print CDF distribution information */
void print_cdf_msg(struct cdf_table_msg *table);

/* get average value of CDF distribution */
double avg_cdf_msg(struct cdf_table_msg *table);

/* Generate a random value based on CDF distribution */
double gen_random_cdf_msg(struct cdf_table_msg *table);

double interpolate_msg(double x, double x1, double y1, double x2, double y2);

/* generate a random floating point number from min to max */
double  rand_range_msg(double min, double max);

double poission_gen_interval_msg(double avg_rate);

void setReqRate(double reqrate);
