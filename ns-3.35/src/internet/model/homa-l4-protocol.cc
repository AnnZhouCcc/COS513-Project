...
HomaL4Protocol::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::HomaL4Protocol")
    ...
    .AddAttribute ("OvercommitLevel", "Minimum number of messages to Grant at the same time",
                   ...
                   MakeUintegerChecker<uint8_t> ())
    .AddAttribute ("m_linkRate", "m_linkRate",
                   DataRateValue (DataRate("10Gbps")),
                   MakeDataRateAccessor (&HomaL4Protocol::m_linkRate),
                   MakeDataRateChecker ())
                     ...
                     uint16_t sport, uint16_t dport, Ptr<Ipv4Route> route)
{
  ...
  TimestampTag a;
         a.SetTimestamp(Simulator::Now().GetNanoSeconds());
       //  message->AddPacketTag(a);
         message->AddByteTag(a);
...
void HomaL4Protocol::ForwardUp (Ptr<Packet> completeMsg,
                                ...
                                Ptr<Ipv4Interface> incomingInterface, uint64_t start)
{
  ...
  m_msgFinishTrace(completeMsg->GetSize(), start,true);
  // TimestampTag a;
  // completeMsg->PeekPacketTag(a);
  // std::cout << a.GetTimestamp() << std::endl;
}
...
//  printf("%u, %u, %u, %u", numPkts, m_msgSizeBytes, m_maxPayloadSize, m_msgSizeBytes / m_maxPayloadSize + (m_msgSizeBytes % m_maxPayloadSize != 0));
  NS_ASSERT(numPkts == m_msgSizeBytes / m_maxPayloadSize + (m_msgSizeBytes % m_maxPayloadSize != 0));
  
  m_maxGrantedIdx = std::min(uint32_t((uint16_t)(m_homa->GetBdp () -1)), numPkts);
          
  // std::cout << "granted " <<  m_maxGrantedIdx << std::endl;
...
Ptr<Packet> HomaOutboundMsg::RemoveNextPktFromTxQ (uint16_t pktOffset)
{
  ...
  if (m_homa->MemIsOptimized ()){
   Ptr<Packet> p = Create<Packet> (m_pktSizes[pktOffset]);
   TimestampTag a;
   a.SetTimestamp(Simulator::Now().GetNanoSeconds());
   p->AddByteTag(a);
...
bool HomaSendScheduler::GetNextPktOfMsg (uint16_t txMsgId, Ptr<Packet> &p)
{
  ...
  if (candidateMsg->GetNextPktOffset(pktOffset))
  {
    ...
    if(!candidateMsg->GetPrio (pktOffset)){
      UnSchedTag tag;
      tag.SetValue(1);
      p->ReplacePacketTag(tag);
    }
    MyPriorityTag mt;
    mt.SetPriority(candidateMsg->GetPrio (pktOffset));
    p->ReplacePacketTag(mt);
...
HomaSendScheduler::TxDataPacket ()
{
  ...
  {   
    bool check = this->GetNextPktOfMsg(nextTxMsgID, p);
    NS_ASSERT(check==true);
                                           ...
                                           Ptr<Ipv4Interface> interface)
{
  ...
  else
  {
    ...
    TimestampTag a;
    bool found = packet->FindFirstMatchingByteTag(a);
    uint64_t now = 0;
...
//    std::cout << "now " << now << std::endl;
    inboundMsg->setMsgStartTime(now);
  }
...
void HomaRecvScheduler::ForwardUp(Ptr<HomaInboundMsg> inboundMsg, int msgIdx)
{
  ...
  m_homa->ForwardUp (inboundMsg->GetReassembledMsg(), 
                     ...
                     inboundMsg->GetIpv4Interface(), inboundMsg->getMsgStartTime());
//  std::cout << "Get " <<  inboundMsg->getMsgStartTime() << std::endl;
