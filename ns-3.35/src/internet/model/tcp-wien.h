...
#ifndef SRC_INTERNET_MODEL_TCP_WIEN_H_
#define SRC_INTERNET_MODEL_TCP_WIEN_H_
...
class TcpWien : public TcpNewReno
{
public:
   ...
   */
  TcpWien (const TcpWien& sock);
  virtual ~TcpWien (void);
...
//                                   const TcpSocketState::TcpCongState_t newState);
  ...
  void UpdateRateHp(Ptr<Packet> packet, const TcpHeader& tcpHeader, Ptr<TcpSocketState> tcb, IntHeader ih, bool fast_react);
  void FastReactHp(Ptr<Packet> packet, const TcpHeader& tcpHeader, Ptr<TcpSocketState> tcb, IntHeader ih);

  void UpdateRateTimely(Ptr<Packet> packet, const TcpHeader& tcpHeader, Ptr<TcpSocketState> tcb, IntHeader ih, bool fast_react);
  void FastReactTimely(Ptr<Packet> packet, const TcpHeader& tcpHeader, Ptr<TcpSocketState> tcb, IntHeader ih);

  void UpdateRateWien(Ptr<Packet> packet, const TcpHeader& tcpHeader, Ptr<TcpSocketState> tcb, IntHeader ih, bool fast_react);
  void FastReactWien(Ptr<Packet> packet, const TcpHeader& tcpHeader, Ptr<TcpSocketState> tcb, IntHeader ih);

  void UpdateRateThetaPowertcp(Ptr<Packet> packet, const TcpHeader& tcpHeader, Ptr<TcpSocketState> tcb, IntHeader ih, bool fast_react);
  void FastReactThetaPowertcp(Ptr<Packet> packet, const TcpHeader& tcpHeader, Ptr<TcpSocketState> tcb, IntHeader ih);



  void SetWienRate(Ptr<TcpSocketState> tcb, DataRate rate, DataRate prevRate);
...
uint32_t numTxBytes;
  ...
  uint64_t wienCounter=0;

  DataRate wienRateAgg;
  DataRate wienRateHop[10]; // 10 hops max.
  uint32_t wienWait=0;
...
#endif /* SRC_INTERNET_MODEL_TCP_WIEN_H_ */
