...
PointToPointNetDevice::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::PointToPointNetDevice")
    ...
    /*Modification*/
    .AddAttribute ("switchNode", 
                   "belongs to switch node or not",
                   BooleanValue (false),
                   MakeBooleanAccessor (&PointToPointNetDevice::switchNode),
                   MakeBooleanChecker ())
    /*Modification*/
...
PointToPointNetDevice::TransmitStart (Ptr<Packet> p)
{
  ...
  //   if(found){
  //     Int.setTelemetryQlenDeq(Int.getHopCount(), numRxBytes-txBytesInt); // queue length at dequeue
